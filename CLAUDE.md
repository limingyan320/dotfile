...
karabiner/           <- (macOS) Karabiner-Elements 键盘映射（Caps Lock 层、双 Shift 切输入法、Cmd⇄Option 全局互换）；setup.sh 把整个目录链到 ~/.config/karabiner（必须目录级链接，GUI 保存会替换单文件 symlink）；automatic_backups/ 已 gitignore

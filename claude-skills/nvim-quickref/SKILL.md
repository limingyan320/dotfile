...
- Codex 以 `codex --yolo --no-alt-screen` 从首次打开时的 git 根启动，是可移动、分屏、关闭并进入 buffer 管理器的普通 listed session buffer；terminal-normal 用 `[a` / `]a` 跳回答、`gx` 打开路径。滚离最新输出后会锁定 cursor / topline，手动滚到底部、按 `G` 或进入 terminal-input 才解锁；最新输出可见时自动 check 完成未读
